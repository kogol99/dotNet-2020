...
namespace Zad2
{
    ...
    class Program
    {
        static void GroupArrangedStudents(List<Student> studentsList, int groupSize)
        {
            var sortedStudents =
            ...
            var groups = sortedStudents
                .Select((s, i) => new { Index = i, Person = s })
                .GroupBy(s => s.Index / groupSize)
                .Select(s => s.Select(v => v.Person).ToList())
                .ToList();

            int groupNumber = 1;
            foreach (List<Student> studentsGroup in groups)
            {
                WriteLine("Grupa numer " + groupNumber++ + ":");
        ...
        static void Main()
        {
            ...
            GroupArrangedStudents(students, 4);
        }
    }
}
